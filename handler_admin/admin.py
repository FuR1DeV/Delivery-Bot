    ...
    async def loading_db(message: types.Message):
        if message.text == "Загрузить БД Заказчиков":
            ...
            with open("logs/table_customers.csv", "w", newline='', encoding="windows-1251") as file:
                ...
                res.insert(0, ("id", "user_id", "username", "telephone", "firstname",
                               "lastname", "money", "rating", "ban"))
...
class AdminOrders:
    ...
    async def enter_orders(message: types.Message):
        ...
        if message.text == "Выгрузить БД отзывов":
            res = global_db_obj.all_orders_reviews()
    ...
    async def order(message: types.Message, state: FSMContext):
        res = admin_get_db_obj.admin_check_order(message.text)
        if res:
            await bot.send_message(message.from_user.id,
    ...
    async def order_details(message: types.Message, state: FSMContext):
        async with state.proxy() as data:
            customer_res = admin_get_db_obj.admin_check_users('customers', data.get("order")[1])
            data["customer"] = customer_res
            performer_res = admin_get_db_obj.admin_check_users('performers', data.get("order")[9])
            data["performer"] = performer_res
            review_res = admin_get_db_obj.admin_check_review(data.get("order")[11])[0]
        ...
        if message.text == "Просмотр Отзывов":
            ...
            await bot.send_message(message.from_user.id,
                                   ...
                                   f"<b>{review_res[1]}</b>\n"
                                   f"Оценка от Заказчика - <b>{review_res[3]}</b>\n"
                                   f"{config.KEYBOARD.get('DASH') * 10}")
            await bot.send_message(message.from_user.id,
                                   f"{config.KEYBOARD.get('DASH') * 10}\n"
                                   f"Отзыв от <b>Исполнителя</b>:\n"
                                   f"<b>{review_res[2]}</b>\n"
                                   f"Оценка от <b>Исполнителя</b> - <b>{review_res[4]}</b>\n"
                                   f"{config.KEYBOARD.get('DASH') * 10}")
        if message.text == "Выгрузить БД этого заказа":
            await bot.send_message(message.from_user.id,
                                   f"Подготовливаем документ")
            try:
                async with state.proxy() as data:
